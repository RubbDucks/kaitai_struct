...
KS project is developed in this repository, with key components in
subdirectories:

* `compiler/` — core compiler (`ksc`)
* `tests/` — test specs, input corpus, and language runners
* `runtime/` — runtime libraries used by generated code
* `doc/` — user and developer documentation
* `visualizer/` — console visualizer (`ksv`, `ksdump`)

NOTE: This repository currently has no git submodules. Clone it normally:

[source,shell]
git clone https://github.com/kaitai-io/kaitai_struct.git

For a canonical contributor happy-path (build compiler, generate formats,
run tests), see <<workflows.adoc#,Contributor & release workflows>>.
...
IMPORTANT: This section is historical and references legacy Bintray
infrastructure that is no longer used.

Use <<workflows.adoc#,Contributor & release workflows>> for current
release preparation and publishing flow.
...
* Historical: Bintray package metadata (no longer active)
