...
std::string EndianToString(Endian endian) {
  return endian == Endian::kLe ? "le" : "be";
}
...
std::string PrimitiveToString(PrimitiveType t) {
  switch (t) {
    case PrimitiveType::kU1: return "u1";
    case PrimitiveType::kU2: return "u2";
    case PrimitiveType::kU4: return "u4";
    case PrimitiveType::kU8: return "u8";
    case PrimitiveType::kS1: return "s1";
    case PrimitiveType::kS2: return "s2";
    case PrimitiveType::kS4: return "s4";
    case PrimitiveType::kS8: return "s8";
    case PrimitiveType::kF4: return "f4";
    case PrimitiveType::kF8: return "f8";
    case PrimitiveType::kStr: return "str";
    case PrimitiveType::kBytes: return "bytes";
  }
  return "u1";
}

ValidationResult PrimitiveFromString(const std::string& text, PrimitiveType* out) {
  if (text == "u1") *out = PrimitiveType::kU1;
  else if (text == "u2") *out = PrimitiveType::kU2;
  else if (text == "u4") *out = PrimitiveType::kU4;
  else if (text == "u8") *out = PrimitiveType::kU8;
  else if (text == "s1") *out = PrimitiveType::kS1;
  else if (text == "s2") *out = PrimitiveType::kS2;
  else if (text == "s4") *out = PrimitiveType::kS4;
  else if (text == "s8") *out = PrimitiveType::kS8;
  else if (text == "f4") *out = PrimitiveType::kF4;
  else if (text == "f8") *out = PrimitiveType::kF8;
  else if (text == "str") *out = PrimitiveType::kStr;
  else if (text == "bytes") *out = PrimitiveType::kBytes;
  else return {false, "invalid primitive type: " + text};
  return {true, ""};
}
...
std::string SerializeExpr(const Expr& expr) {
  ...
  switch (expr.kind) {
    ...
    case Expr::Kind::kBinary:
      out << "(bin " << std::quoted(expr.text) << " " << SerializeExpr(*expr.lhs) << " " << SerializeExpr(*expr.rhs) << ")";
      return out.str();
  }
  return "";
}
...
struct ExprParser {
  ...
  ValidationResult Parse(Expr* out) {
    SkipWs();
    if (Next() != '(') return {false, "expression must start with '('"};
    std::string tag;
    if (!ReadToken(&tag)) return {false, "missing expression tag"};
    if (tag == "int") {
      std::string num;
      if (!ReadToken(&num)) return {false, "missing int literal"};
      out->kind = Expr::Kind::kInt;
      out->int_value = std::stoll(num);
    } else if (tag == "bool") {
      std::string val;
      if (!ReadToken(&val)) return {false, "missing bool literal"};
      out->kind = Expr::Kind::kBool;
      out->bool_value = val == "true";
      if (val != "true" && val != "false") return {false, "invalid bool literal: " + val};
    } else if (tag == "name") {
      std::string val;
      if (!ReadQuoted(&val)) return {false, "invalid name literal"};
      out->kind = Expr::Kind::kName;
      out->text = val;
    } else if (tag == "un") {
      std::string op;
      if (!ReadQuoted(&op)) return {false, "invalid unary operator"};
      Expr operand;
      ...
      *out = Expr::Unary(op, std::move(operand));
      ...
      std::string op;
      if (!ReadQuoted(&op)) return {false, "invalid binary operator"};
      Expr left;
    ...
    SkipWs();
    if (Next() != ')') return {false, "expression missing closing ')'"};
    return {true, ""};
  }
};
...
ValidationResult Validate(const Spec& spec) {
  ...
  for (const auto& t : spec.types) {
    ...
    if (t.type.kind == TypeRef::Kind::kUser && t.type.user_type.empty()) {
  ...
  for (const auto& attr : spec.attrs) {
    ...
    if (attr.type.kind == TypeRef::Kind::kUser && attr.type.user_type.empty()) {
      return {false, "attr user type reference requires user_type"};
    }
  }
...
std::string Serialize(const Spec& spec) {
  ...
  for (const auto& v : spec.validations) {
    out << "validation " << std::quoted(v.target) << " " << std::quoted(SerializeExpr(v.condition_expr)) << " "
        << std::quoted(v.message) << "\n";
  }
  out << "end\n";
...
ValidationResult Deserialize(const std::string& encoded, Spec* out) {
  ...
  if (!std::getline(in, line)) return {false, "missing spec name line"};
  {
  ...
  if (!std::getline(in, line)) return {false, "missing default endian line"};
  {
    std::istringstream row(line);
    std::string key;
  ...
  auto parse_count = [&](const std::string& expected, size_t* count) -> ValidationResult {
    if (!std::getline(in, line)) return {false, "missing section header: " + expected};
    std::istringstream row(line);
    std::string key;
  ...
  for (size_t i = 0; i < count; i++) {
    if (!std::getline(in, line)) return {false, "truncated type section"};
    std::istringstream row(line);
    std::string key;
    TypeDef type;
  ...
  out->attrs.clear();
  for (size_t i = 0; i < count; i++) {
    if (!std::getline(in, line)) return {false, "truncated attr section"};
    std::istringstream row(line);
    std::string key;
    Attr attr;
    std::string endian_text;
  ...
  out->instances.clear();
  for (size_t i = 0; i < count; i++) {
    if (!std::getline(in, line)) return {false, "truncated instance section"};
    std::istringstream row(line);
    std::string key;
    Instance inst;
  ...
  for (size_t i = 0; i < count; i++) {
    if (!std::getline(in, line)) return {false, "truncated validation section"};
    std::istringstream row(line);
    std::string key;
    Validation val;
    std::string expr_text;
    if (!(row >> key >> std::quoted(val.target) >> std::quoted(expr_text) >> std::quoted(val.message)) || key != "validation") {
