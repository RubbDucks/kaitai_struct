...
- On Windows/MSYS, `tests/run-ruby` may fail immediately with `rspec.bat: command not found` if Ruby RSpec is not installed; install RSpec (so `rspec`/`rspec.bat` is on `PATH`) before running Ruby specs.
- After editing `compiler/shared/src/main/scala/io/kaitai/struct/ir/MigrationIr.scala`, rebuild the staged Scala launcher (`compilerJVM/stage`) before running `tests/migration_golden/run_cpp17_differential.py`; otherwise emitted `.ksir` files still use stale IR serialization logic.
